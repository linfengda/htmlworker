...
public class CssUtils {
	...
	/**
	 * 处理CSS属性: border-width/border-style/border-color/margin/padding等属性
	 * 将属性值分解为top, bottom, left, right
	 * 
	 * @param box	属性值
	 * @param pre	属性前缀
	 * @param post	属性后缀
	 * @return
	 */
    public Map<String, String> parseBoxValues(final String box, final String pre, final String post) {
        return parseBoxValues(box, pre, post, null);
    }
    
    public Map<String, String> parseBoxValues(final String box, final String pre, final String post, String preKey) {
		String[] props = box.split(" ");
		int length = props.length;
		Map<String, String> map = new LinkedHashMap<String, String>(4);
		if (length == 1) {					//border-width: 20px，表示top/bottom/left/right都为20px。 
		...
		} else if (length == 2) {			//border-width: 20px 10px，表示top/bottom: 20px, left/right: 10px
            if (preKey == null) {
		...
		} else if (length == 3) {			//border-width: 20px 30px 10px，表示top: 20px, left/right: 30px, bottom: 10px
            if (preKey == null) {
                map.put(MessageFormat.format(_0_TOP_1, pre, post), props[0]);
		...
		} else if (length == 4) {			//border-width: 20px 30px 10px 10px，表示top: 20px, right: 30px, bottom: 10px, left: 10px
            if (preKey == null) {
                map.put(MessageFormat.format(_0_TOP_1, pre, post), props[0]);
                map.put(MessageFormat.format(_0_BOTTOM_1, pre, post), props[2]);
                map.put(MessageFormat.format(_0_RIGHT_1, pre, post), props[1]);
    ...
    /**
     * 处理CSS属性: border/border-top/border-bottom/border-left/border-right
     * 
     * @param border		border值
     * @param borderKey		borderKey名
     * @return				box属性K-V(width, style, color)
     */
