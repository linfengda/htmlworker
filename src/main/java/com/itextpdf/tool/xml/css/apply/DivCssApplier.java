...
public class DivCssApplier implements CssApplier<PdfDiv> {
    ...
    public PdfDiv apply(final PdfDiv div, final Tag t, final MarginMemory memory, final PageSizeContainable psc, HtmlPipelineContext context) {
        ...
        if (widthValue != null) {
            ...
            if (utils.isNumericValue(widthValue) || utils.isMetricValue(widthValue)) {
            	float ptWidth = utils.parsePxInCmMmPcToPt(widthValue);
            	float minWidth = Math.min(pageWidth, ptWidth);
            	div.setWidth(minWidth);
        ...
        //support css margin: 0 auto
        if (isCenterDiv(t)) {
        	float pageWidth = psc.getPageSize().getWidth();
        	Float divWidth = div.getWidth();
        	Float divHight = div.getHeight();
        	if (divWidth != null) {
        		if (divWidth > 500) {
        			divWidth = divWidth*0.9f;
        			divHight = divHight*0.8f;
    			}else{
    				divWidth = divWidth*1.0f;
        			divHight = divHight*0.8f;
    			}
        		div.setWidth(divWidth);
        		div.setHeight(divHight);
    			div.setPaddingLeft((pageWidth - divWidth)/1.5f);
	    	}
        }
    ...
    private boolean isCenterDiv(Tag t){
		String margin_top = t.getCSS().get("margin-top");
		String margin_bottom = t.getCSS().get("margin-bottom");
		String margin_left = t.getCSS().get("margin-left");
		String margin_right = t.getCSS().get("margin-right");
		if (margin_top != null && margin_bottom != null && margin_left != null && margin_right != null) {
			if (margin_top.startsWith("0") && margin_bottom.startsWith("0") && margin_left.equals("auto") && margin_right.equals("auto")) {
				return true;
			}
		}
		return false;
