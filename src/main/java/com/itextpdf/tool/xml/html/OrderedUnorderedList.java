		...
		if (size > 0) {
			...
			try {
				htmlPipelineContext = getHtmlPipelineContext(ctx);
					list = (com.itextpdf.text.List) getCssApplyService().apply(new com.itextpdf.text.List(), tag, htmlPipelineContext);
				} catch (NoCustomContextException e) {
				list =  (com.itextpdf.text.List) getCssApplyService().apply(new com.itextpdf.text.List(), tag, null);
			}
			...
			for (Element li : listElements) {
				if (li instanceof ListItem) {
					...
					try {
						list.add(getCssApplyService().apply(li, child, getHtmlPipelineContext(ctx)));
