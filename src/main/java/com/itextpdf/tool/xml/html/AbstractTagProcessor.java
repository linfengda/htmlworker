...
public abstract class AbstractTagProcessor implements TagProcessor, CssAppliersAware {
	...
	public CSSResolver getCSSResolver(final WorkerContext context) throws NoCustomContextException {
		return (CSSResolver)context.get(CssResolverPipeline.class.getName());
	}
	...
	public final List<Element> startElement(final WorkerContext ctx, final Tag tag) {
        ...
        list = beforeStartElement(ctx, tag, list);
		list.addAll(start(ctx, tag));
		return list;
	}
	
	/**
	 * any children tag class can override this method, to do something special before startElement()
	 * @param ctx
	 * @param tag
	 * @param list
	 * @return
    ...
    protected List<Element> textContent(final WorkerContext ctx, final Tag tag, final String content) {
        ...
        for (Chunk sanitized : sanitizedChunks) {
            try {
                l.add(getCssApplyService().apply(sanitized, tag, getHtmlPipelineContext(ctx)));
		...
		try {
			...
			if (currentContent.size() > 0) {
				if (addNewLines) {
                    ...
                    if (p.trim()) {
                        if (applyCSS) {
                            p = (Paragraph) getCssApplyService().apply(p, tag, getHtmlPipelineContext(ctx));
                        }
					...
					p = (NoNewLineParagraph) getCssApplyService().apply(p, tag, getHtmlPipelineContext(ctx));
                    if (direction == PdfWriter.RUN_DIRECTION_RTL) {
                        doRtlIndentCorrections(p);
                        invertTextAlignForParagraph(p);
                    }
	 ...
	 */
	public void setCssApplyService(final CssApplyService cssAppliers) {
		this.cssApplyService = cssAppliers;
	}
	 ...
	 */
	public CssApplyService getCssApplyService() {
		return cssApplyService;
	}
