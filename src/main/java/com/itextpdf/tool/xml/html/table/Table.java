				...
				if (baseLevel instanceof Phrase) {
					for (int i = 0; i < ((Phrase) baseLevel).size(); i++) {
						...
						if (inner instanceof Chunk) {
							...
							ChunkCssApplier chunkCssApplier = (ChunkCssApplier) getCssApplyService().getCssApplier(Chunk.class);
							float widestWord = startWidth + widthDeviation + chunkCssApplier.getWidestWord((Chunk) inner);
					...
					for (Element li : ((com.itextpdf.text.List) baseLevel).getItems()) {
						...
						for (Chunk c : li.getChunks()) {
							...
							ChunkCssApplier chunkCssApplier = (ChunkCssApplier) getCssApplyService().getCssApplier(Chunk.class);
							float widestWord = chunkCssApplier.getWidestWord(c);
                ...
                for (Element inner : p) {
                    ...
                    if (inner instanceof Chunk) {
                        ...
                        if (chunkAttributes != null && chunkAttributes.containsKey(Chunk.IMAGE)) {
                        	...
                        	ChunkCssApplier chunkCssApplier = (ChunkCssApplier) getCssApplyService().getCssApplier(Chunk.class);
                            widestWord = chunkCssApplier.getWidestWord((Chunk) inner);
                        }
                    }
