...
public class ParaGraph extends AbstractTagProcessor {
	...
	@Override
	public List<Element> beforeStartElement(WorkerContext ctx, Tag tag, List<Element> list){
		...
		String style = tag.getAttributes().get("style");
		if (null != style && style.indexOf("page-break-before: always") != -1) {
			list.add(Chunk.NEXTPAGE);
		}
		String newline = tag.getAttributes().get("newline");
		if (null != newline && newline.equalsIgnoreCase("true")) {
        ...
        if (currentContent.size() > 0) {
            ...
            for (Element e : currentContent) {
                if (e instanceof ListItem) { 			
                    if (!elements.isEmpty()) {
                        processElements(ctx, tag, elements, l);
                        elements.clear();
                    }
    ...
    protected void processElements(final WorkerContext ctx, final Tag tag, final List<Element> paragraphElements, List<Element> l) {
        Paragraph p = new Paragraph();
        p.setMultipliedLeading(1.2f);
        Map<String, String> css = tag.getCSS();
        if (null != css.get(CSS.Property.TAB_INTERVAL)) {
            addTabIntervalContent(ctx, tag, paragraphElements, p, css.get(CSS.Property.TAB_INTERVAL));
            l.add(p);
        } else if (null != css.get(CSS.Property.TAB_STOPS)) {
            addTabStopsContent(paragraphElements, p, css.get(CSS.Property.TAB_STOPS));
            l.add(p);
        } else if (null != css.get(CSS.Property.XFA_TAB_STOPS)) {
            addTabStopsContent(paragraphElements, p, css.get(CSS.Property.XFA_TAB_STOPS));
            l.add(p);                                                                    
        } else {
            List<Element> pList = currentContentToParagraph(paragraphElements, true, true, tag, ctx);
            for (Element e : pList) {
                l.add(e);
            }
        }
    ...
    protected void processListItems(final WorkerContext ctx, final Tag tag, final List<ListItem> listItems, List<Element> l) {
        try {
            ...
            list = (com.itextpdf.text.List) getCssApplyService().apply(list, tag, getHtmlPipelineContext(ctx));
            list.setIndentationLeft(0);
            int i = 0;
            for (ListItem li : listItems) {
                li = (ListItem) getCssApplyService().apply(li, tag, getHtmlPipelineContext(ctx));
			...
			if (e instanceof TabbedChunk) {
                ...
                if (e instanceof LineSeparator) {
                    try {
                        ...
                        Chunk newLine = (Chunk)getCssApplyService().apply(new Chunk(Chunk.NEWLINE), tag, htmlPipelineContext);
                        p.add(newLine);
