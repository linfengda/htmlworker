		...
		if (null != src && src.length() > 0) {
			...
			try {
				...
				if (src.startsWith("data:image/")) {
					String data = src.substring(src.indexOf("base64,")+7);
					img = com.itextpdf.text.Image.getInstance(new BASE64Decoder().decodeBuffer(data));
				}else{
				...
				throw new RuntimeWorkerException(LocaleMessages.getInstance().getMessage(LocaleMessages.HTML_IMG_DECODE_FAIL), e);
			...
			if (null != img) {
				try {
					...
					l.add(getCssApplyService().apply(new Chunk((com.itextpdf.text.Image) getCssApplyService().apply(img, tag, htmlPipelineContext), 0, 0, true), tag, htmlPipelineContext));
				} catch (NoCustomContextException e) {
