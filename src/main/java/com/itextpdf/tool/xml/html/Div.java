...
public class Div extends AbstractTagProcessor {
	...
	public List<Element> content(final WorkerContext ctx, final Tag tag, final String content) {
        ...
        try {
            ...
            for (Chunk sanitized : sanitizedChunks) {
                noNewLineParagraph.add(getCssApplyService().apply(sanitized, tag, hpc));
            }
            if (noNewLineParagraph.size() > 0) {
                l.add(getCssApplyService().apply(noNewLineParagraph, tag, hpc));
            }
	...
	public List<Element> end(final WorkerContext ctx, final Tag tag, final List<Element> currentContent) {
		try {
			PdfDiv div = (PdfDiv)getCssApplyService().apply(new PdfDiv(), tag, getHtmlPipelineContext(ctx));
