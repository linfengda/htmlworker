...
public class TagAttributeState implements State {
	 ...
	 */
	public TagAttributeState(final HTMLParser parser) {
		this.parser = parser;
	}
	...
	public void process(final char character) {
		if (character == '/') {
			...
			parser.stateController().selfClosing();
		} else if (character == '=') {
			this.parser.memory().setCurrentAttr(this.parser.memory().textBuffString());
			this.parser.memory().resetTextBuffer();
			this.parser.stateController().attributeValue();
	...
	protected boolean setAttribute() {
		String attr = this.parser.memory().textBuffString();
		if (attr.length() > 0) {
			this.parser.memory().setCurrentAttr(attr);
			this.parser.memory().resetTextBuffer();
			return true;
		}
		return false;
