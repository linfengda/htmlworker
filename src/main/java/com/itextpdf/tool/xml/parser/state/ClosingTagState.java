...
public class ClosingTagState implements State {
	 ...
	 */
	public ClosingTagState(final HTMLParser parser) {
		this.parser = parser;
	}

	public void process(final char character) {
		if (character == '>') {
			this.parser.memory().setCurrentTag(this.parser.memory().textBuffString());
			this.parser.endElement();
			this.parser.memory().resetTextBuffer();
			this.parser.memory().setCurrentNameSpace("");
			this.parser.stateController().inTag();
		} else if (character == ':') {
			this.parser.memory().setCurrentNameSpace(this.parser.memory().textBuffString());
			this.parser.memory().resetTextBuffer();
