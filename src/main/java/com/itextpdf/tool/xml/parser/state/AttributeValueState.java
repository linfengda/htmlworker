...
public class AttributeValueState implements State {
	 ...
	 */
	public AttributeValueState(final HTMLParser parser) {
		this.parser = parser;
	}
	...
	public void process(final char character) {
		if (character == '\'') {
			this.parser.stateController().singleQuotedAttr();
		} else if (character == '"') {
			this.parser.stateController().doubleQuotedAttr();
		} else if (!Character.isWhitespace((char) character)){
			this.parser.memory().append(character);
			this.parser.stateController().unquotedAttr();
		}
	}
